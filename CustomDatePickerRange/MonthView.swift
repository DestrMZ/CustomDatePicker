...
#Preview {
    MonthView(calendarManager: CalendarManager(minimumDate: Date(), maximumDate: Date().addingTimeInterval(60 * 60 * 24 * 365), isFutureSelectionEnabled: false), monthOffset: 0)
}
...
extension MonthView {
    ...
    func isEnabled(date: Date) -> Bool {
        ...
        // Если разрешен выбор будущих дат
        if calendarManager.isFutureSelectionEnabled {
            // Дата должна быть больше или равна текущей
            if calendarManager.calendar.compare(clampedDate, to: today, toGranularity: .day) == .orderedAscending {
                return false
            }
        } else {
            // Если выбор будущих дат не разрешен, дата должна быть меньше или равна текущей
            if calendarManager.calendar.compare(clampedDate, to: today, toGranularity: .day) == .orderedDescending {
                return false
            }
        }

        // Проверяем, что дата находится в допустимом диапазоне между minimumDate и maximumDate
        ...
        return !isOneOfDisabledDates(date: clampedDate)  // Проверяем, не является ли дата в списке недоступных
    }
    ...
    func dateTapped(date: Date) {
        if isEnabled(date: date) {
            if calendarManager.startDate == nil {  // Если дата начала еще не выбрана
                calendarManager.startDate = date
                calendarManager.endDate = nil  // Очистить конечную дату, если она была установлена ранее
            } else {
